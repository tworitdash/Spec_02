...
for p = 1:size(th , 2)
   ...
   [mx, my] = meshgrid(-10:1:10, -10:1:10);
    ...
    yin = Constant_term .* SGFxx .* abs(I_kxm).^2 .* abs(J_kym).^2;
    ...
    AEP_x(p) = 1j .* kz .* SGFxx(11, 11) .* I_kxm_1 .* J_kym_1 .* exp(-1j * k0 .* r_obs)/(2 * pi * r_obs) .* I0;
    AEP_y(p) = 1j .* kz .* SGFyx(11, 11) .* I_kxm_1 .* J_kym_1 .* exp(-1j * k0 .* r_obs)/(2 * pi * r_obs) .* I0;
    AEP_z(p) = 1j .* kz .* SGFzx(11, 11) .* I_kxm_1 .* J_kym_1 .* exp(-1j * k0 .* r_obs)/(2 * pi * r_obs) .* I0;
...
%  ylabel('AEP (dB) \phi = 0 rad', 'FontSize', 12, 'FontWeight', 'bold');
%  title('AEP on E plane', 'FontSize', 12, 'FontWeight', 'bold');
grid on;
%ylim([-30 0]);
%print('AEP_90', '-depsc');
