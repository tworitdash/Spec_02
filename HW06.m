...
f = 10e9;
   ...
   [mx, my] = meshgrid(-5:1:4, -5:1:4);
    ...
    e_single_x = Kons .* SGFxx .* B;
    e_single_y = Kons .* SGFyx .* B;
    e_single_z = Kons .* SGFzx .* B;
    
    e_single_abs = sqrt(e_single_x.^2 + e_single_y.^2 + e_single_z.^2);
    
    e_single_th = e_single_x .* cos(theta) .* cos(phi) + e_single_y .* cos(theta) .* sin(phi) - e_single_z .* sin(theta);
    e_single_ph = -sin(phi) .* e_single_x + cos(phi) .* e_single_y;
    
    e_single_abs_2 = sqrt(e_single_th.^2 + e_single_ph.^2);
    
     max_single_th = abs(e_single_th(1, 91));
     max_single_ph = abs(e_single_ph(91, 91));
...
%     figure(1);
%      plot(theta(1, :)*(180/pi), db(abs(e_single_th(1, :))./max_single_th), 'LineWidth', 2);
%     hold on;
%      plot(theta(1, :)*(180/pi), db(abs(e_single_ph(91, :))./max_single_ph), 'LineWidth', 2);
     
     
     max_th = abs(e_th(1, 91));
     max_ph = abs(e_ph(91, 91));
    
%     figure(2);
%     plot(theta(1, :)*(180/pi), db(abs(e_th(1, :))/max_th), '--', 'LineWidth', 2);
%     hold on;
...
%     ylabel('E_{\theta}(\phi = 0), E_{\phi}(\phi = \pi/2) (dB)', 'FontSize', 12, 'FontWeight', 'bold');
%     title('Far electric field pattern', 'FontSize', 12, 'FontWeight', 'bold');
%     legend({'E_{\theta}(\phi = 0)','E_{\phi}(\phi = \pi/2)'},'Location','northeast', 'FontSize', 12, 'FontWeight', 'bold');
