...
lambda = c./f;
zeta = 120*pi;
epsilon_r = linspace(1+eps, 25, 50);
h = 15e-3;
...
[D_krho_TE] = D_kro_leak(k0, "TE", epsilon_r(m), h, hs, zeta, zeta_s, krho);
[D_krho_TM] = D_kro_leak(k0, "TM", epsilon_r(m), h, hs, zeta, zeta_s, krho);
...
[krho_TE, f_axis_1] = finddrop_I4(k0, epsilon_r(m), krho_g_TE, h, hs, zeta, zeta_s, "TE");
[krho_TM, f_axis_2] = finddrop_I4(k0, epsilon_r(m), krho_g_TM, h, hs, zeta, zeta_s, "TM");

krho_TE_e(m) = krho_TE(f_axis_1 == f1) .* k1;
krho_TM_e(m) = krho_TM(f_axis_1 == f1) .* k1;
...
kz0_TE = (k1./(pi.*epsilon_r(m).*(2.*hb).^2)) .* ((2.*pi.*hb.*epsilon_r(m) + 1j)./(1 + (1./(2.*pi.*hb.*epsilon_r(m)).^2)));
kz0_TM = (k1./(4 .* hb)).*(1 + sqrt(1 + 8j .* (hb./(pi.*epsilon_r(m)))));
