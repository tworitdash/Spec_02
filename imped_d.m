...
for i = 1:length(d)
    ...
    F1 = F(z1, L2, L1, d(i), b);                   % evaluate integrand at Gauss-Legendre points
    F2 = F(z2, L2, L1, d(i), b);
    F3 = F(z3, L2, L1, d(i), b);
                                                % sum integrals over the three subintervals
    Int = w1'*F1 + w2'*F2 + w1'*F3;             % note w3 = w1 by symmetry
    
    Z21m(i) = 1j * eta * Int / (4*pi);
    Z21(i) = Z21m(i) / (sin(pi*L1) * sin(pi*L2));

end

figure()
plot(d, real(Z21),d, imag(Z21),'LineWidth',2)
